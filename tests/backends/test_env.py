...
# Short, collision-resistant prefix used for all tests.
PREFIX = "FCTEST_"
...
async def test_static_is_default_type(monkeypatch):
    ...
    cred = await EnvCredentialBackend(prefix=PREFIX).resolve()
...
async def test_static_missing_api_key_raises(monkeypatch):
    ...
    with pytest.raises(CredentialError, match=f"{PREFIX}API_KEY"):
        await EnvCredentialBackend(prefix=PREFIX).resolve()
